...
func _ready():
	for iterated_day in days:
		for card in iterated_day.work_cards:
			card.shown = false
			card.used = false
		for card in iterated_day.leisure_cards:
			card.shown = false
			card.used = false
...
func trigger_game_over(event: Event):
	clear_draft()
	game_over_instance = game_over_scene.instantiate()
	game_over_instance.set_reason(event.details)
	game_over_instance.connect_signal(restart_game)
	show_cards(event.cards)
...
func show_game_over():
	clear_draft()
	%Overlay.show()
	$CanvasLayer2.add_child(game_over_instance)
